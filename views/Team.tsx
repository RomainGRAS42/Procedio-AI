  ...
  useEffect(() => {
    if (isActive && buttonRef.current) {
      ...
      setMenuPosition({
        ...
        left: rect.right + window.scrollX - 224, // Align right (w-56 = 224px)
      });
    }
  }, [isActive]);
  ...
  return (
    <>
      <button
        ...
        onClick={(e) => {
          e.stopPropagation();
          onToggle(isActive ? null : member.id);
        }}
        className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all active:scale-95 z-10 relative"
        aria-label={`Gérer l'utilisateur ${member.first_name}`}
        aria-haspopup="true"
      ...
      {isActive && createPortal(
        <div 
          className="absolute z-[9999] w-56 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-200 origin-top-right"
          style={{ top: menuPosition.top, left: menuPosition.left }}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">
              Actions pour {member.first_name}
            </p>
          </div>
          <button
            onClick={(e) => {
            ...
            <i className="fa-solid fa-eye w-4 text-center"></i>
            Voir détails
          </button>
          <button
            onClick={(e) => {
                          ...
                          <td className="p-6 text-right relative">
                            <div className="flex items-center justify-end gap-2">
                              {user.id !== member.id && (
                                <ActionMenu 
                                  member={member}
                                  isActive={activeMenuId === member.id}
                                  onToggle={(id) => setActiveMenuId(id)}
                                  onViewDetails={() => handleToggleExpand(member.id)}
                                  onResetPassword={() => handleResetPassword(member.email)}
                                />
                              )}
                              {user.id === member.id && (
                                <span className="text-[10px] font-bold text-slate-300 italic px-4 py-2">Vous</span>
                              )}
                            </div>
                          </td>
                        </tr>
