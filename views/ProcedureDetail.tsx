...
const ProcedureDetail: React.FC<ProcedureDetailProps> = ({
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`procedure_chat_${procedure.id}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
          ...
          filter: `procedure_id=eq.${procedure.db_id || procedure.uuid}`,
        }, // Filter by procedure to avoid noise
        async (payload) => {
    ...
    const resolveDocUrl = async () => {
      ...
      if (path) {
        if (path.startsWith("http")) {
          ...
          const { data } = await supabase.storage.from("procedures").createSignedUrl(path, 3600);
  ...
  const handleSendMessage = async (textOverride?: string) => {
    ...
    try {
      ...
      let aiResponseText =
        data.output || data.text || (typeof data === "string" ? data : "Analyse terminée.");

      // Remplacement du préfixe robotique par quelque chose de plus naturel
      aiResponseText = aiResponseText.replace(
        /D'après le CONTEXTE fourni, voici la réponse précise à ta question/gi,
        "Voici ce que j'ai trouvé dans le document"
      );
      aiResponseText = aiResponseText.replace(
        /D'après le CONTEXTE fourni,/gi,
        "Selon la procédure,"
      );
      ...
      >
        <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div
              className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-xl transition-colors duration-300 ${isHumanChat ? "bg-emerald-500" : "bg-indigo-600"}`}>
            ...
            <div>
              ...
              <p
                className={`text-[9px] font-black uppercase tracking-widest mt-0.5 ${isHumanChat ? "text-emerald-600" : "text-emerald-500"}`}>
              ...
              {directMessages.length === 0 && (
                <div className="flex flex-col items-center justify-center py-10 text-slate-400 opacity-60">
                  ...
                  <p className="text-xs font-medium text-center max-w-[200px]">
                    Posez votre question directement à{" "}
                    {referentExpert?.first_name || "votre référent"}. <br />
        ...
        <div className="p-6 bg-white border-t border-slate-50 space-y-4">
          ...
          <div className="relative">
            <input
              ...
              placeholder={
                isHumanChat
                  ? `Écrivez à ${referentExpert?.first_name || "votre référent"}...`
                  : "Posez votre question sur ce document..."
              }
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && (isHumanChat ? handleSendDirectMessage() : handleSendMessage())
              }
              disabled={isTyping}
            />
            <button
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button
              ...
              <div className="flex flex-col items-start leading-none gap-0.5">
                ...
                <span className="hidden sm:inline text-[8px] opacity-70 italic normal-case tracking-normal">
                  (Lien valide 1h)
                </span>
              </div>
            </button>
