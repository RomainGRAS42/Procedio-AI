...
const Missions: React.FC<MissionsProps> = ({ user, onSelectProcedure, setActiveTransfer }) => {
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto space-y-12 animate-fade-in pb-20">
      ...
      {showCreateModal &&
        createPortal(
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-8 bg-slate-900/60 backdrop-blur-md animate-fade-in">
            <div className="bg-white rounded-[3rem] w-full max-w-[95vw] xl:max-w-[1400px] shadow-2xl animate-scale-up flex flex-col max-h-[92vh] overflow-hidden border border-white/20">
              <div className="flex items-center justify-between p-8 pb-4 shrink-0 border-b border-slate-50">
                ...
                <button
                  ...
                  className="w-12 h-12 rounded-full bg-slate-50 text-slate-300 hover:bg-rose-50 hover:text-rose-500 transition-all flex items-center justify-center">
                ...
                <div className="grid grid-cols-12 gap-8 lg:gap-12">
                    ...
                    <div className="col-span-12">
                        ...
                        <input
                            ...
                            className="w-full p-5 bg-slate-50 border border-slate-100 rounded-2xl focus:bg-white focus:border-indigo-500 outline-none transition-all font-black text-slate-800 text-xl placeholder:font-medium placeholder:text-slate-300"
                    ...
                    <div className="col-span-12 lg:col-span-5 space-y-6">
                        <div className="space-y-2 h-full flex flex-col">
                          ...
                          <textarea
                            ...
                            className="w-full flex-1 min-h-[200px] p-6 bg-slate-50 border border-slate-100 rounded-3xl focus:bg-white focus:border-indigo-500 outline-none transition-all font-medium text-slate-600 resize-none leading-relaxed text-sm"
                            value={newMission.description}
                            onChange={(e) => setNewMission({ ...newMission, description: e.target.value })}
                          />
                        </div>

                        {/* Toggle: Needs Attachment */}
                        <div
                          className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex items-center justify-between group/toggle hover:border-indigo-200 transition-all cursor-pointer"
                          onClick={() =>
                            setNewMission({ ...newMission, needs_attachment: !newMission.needs_attachment })
                          }>
                          <div className="flex items-center gap-4">
                            <div
                              className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg transition-all ${newMission.needs_attachment ? "bg-indigo-600 text-white shadow-lg shadow-indigo-200" : "bg-white text-slate-300 border border-slate-200"}`}>
                            ...
                            <div>
                              ...
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                Fichier requis pour valider
                              </p>
                            </div>
                          </div>
                          <div
                            className={`w-14 h-7 rounded-full transition-all relative ${newMission.needs_attachment ? "bg-indigo-600" : "bg-slate-200"}`}>
                            <div
                              className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all shadow-sm ${newMission.needs_attachment ? "right-1" : "left-1"}`}></div>
                          </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-3">
                             ...
                             <div className="grid grid-cols-3 gap-3">
                                <button
                                  ...
                                  className={`p-4 rounded-3xl border transition-all flex flex-col items-center gap-2 ${newMission.mission_type === 'solo' ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-200 scale-105' : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-white hover:border-indigo-200'}`}
                                >
                                   <i className="fa-solid fa-user text-lg"></i>
                                   <span className="text-[9px] font-black uppercase tracking-widest">Solo</span>
                                </button>
                                <button
                                  type="button"
                                  onClick={() => setNewMission({ ...newMission, mission_type: 'team' })}
                                  className={`p-4 rounded-3xl border transition-all flex flex-col items-center gap-2 ${newMission.mission_type === 'team' ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-200 scale-105' : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-white hover:border-indigo-200'}`}
                                >
                                   <i className="fa-solid fa-users text-lg"></i>
                                   <span className="text-[9px] font-black uppercase tracking-widest">Équipe</span>
                                </button>
                                <button
                                  type="button"
                                  onClick={() => setNewMission({ ...newMission, mission_type: 'challenge' })}
                                  className={`p-4 rounded-3xl border transition-all flex flex-col items-center gap-2 ${newMission.mission_type === 'challenge' ? 'bg-amber-500 border-amber-500 text-white shadow-lg shadow-amber-200 scale-105' : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-white hover:border-amber-200'}`}
                                >
                                   <i className="fa-solid fa-trophy text-lg"></i>
                                   <span className="text-[9px] font-black uppercase tracking-widest">Défis</span>
                                </button>
                             </div>
                        </div>
                        ...
                        <div className="space-y-3">
                              ...
                              {newMission.mission_type === 'solo' && (
                                  <div className="relative group">
                                    <div className="absolute left-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs border border-indigo-100">
                                        <i className="fa-solid fa-user-check"></i>
                                    </div>
                                    <select
                                        className="w-full p-5 pl-14 bg-slate-50 border border-slate-100 rounded-3xl focus:bg-white focus:border-indigo-500 outline-none transition-all font-bold text-slate-700 appearance-none cursor-pointer text-sm"
                                    ...
                                    <div className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none group-hover:text-indigo-600 transition-colors">
                                        <i className="fa-solid fa-chevron-down text-xs"></i>
                                    </div>
                                  </div>
                              )}
                              ...
                              {newMission.mission_type === 'team' && (
                                  <div className="bg-slate-50 border border-slate-100 rounded-3xl p-4 h-[220px] overflow-y-auto space-y-2 custom-scrollbar">
                                     {technicians.map((tech) => (
                                        <label key={tech.id} className="flex items-center gap-3 cursor-pointer group select-none hover:bg-white p-3 rounded-2xl transition-all border border-transparent hover:border-slate-100 hover:shadow-sm">
                                           <div className={`w-6 h-6 rounded-lg border flex items-center justify-center transition-all ${newMission.participants.includes(tech.id) ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-300 text-transparent'}`}>
                                           ...
                                           <div className="flex flex-col">
                                               <span className={`text-sm font-bold transition-colors ${newMission.participants.includes(tech.id) ? 'text-indigo-900' : 'text-slate-600 group-hover:text-indigo-600'}`}>
                                                  {tech.first_name} {tech.last_name}
                                               </span>
                              ...
                              {newMission.mission_type === 'challenge' && (
                                  <div className="p-8 bg-amber-50 rounded-3xl border border-amber-100 flex flex-col items-center justify-center gap-4 text-amber-600 text-center h-[220px]">
                                     <div className="w-16 h-16 rounded-full bg-white shadow-lg shadow-amber-200 flex items-center justify-center text-2xl animate-pulse">
                                         <i className="fa-solid fa-bolt"></i>
                                     </div>
                                     <div>
                                        <p className="text-sm font-black uppercase tracking-tight">Premier arrivé</p>
                                        <p className="text-xs font-bold opacity-80 uppercase tracking-widest">Premier servi</p>
                                     </div>
                                  </div>
                              )}
                        </div>
                    ...
                    {/* COL 3: Settings (XP, Urgency, Recurrence, Deadline) */}
                    <div className="col-span-12 lg:col-span-3 space-y-6">
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <input
                                type="number"
                                className="w-full p-5 pl-14 bg-slate-50 border border-slate-100 rounded-3xl focus:bg-white focus:border-indigo-500 outline-none transition-all font-black text-indigo-600 text-xl"
                                value={newMission.xp_reward}
                                onChange={(e) =>
                                    setNewMission({ ...newMission, xp_reward: parseInt(e.target.value) })
                                }
                                />
                                <div className="absolute left-5 top-1/2 -translate-y-1/2 text-indigo-300 text-lg group-hover:scale-110 transition-transform">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <div className="absolute left-5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white flex items-center justify-center text-xs shadow-sm text-slate-400">
                                    <i className="fa-solid fa-fire"></i>
                                </div>
                                <select
                                className="w-full p-5 pl-16 bg-slate-50 border border-slate-100 rounded-3xl focus:bg-white focus:border-indigo-500 outline-none transition-all font-bold text-slate-700 appearance-none cursor-pointer text-xs"
                        ...
                        <div className="pt-2 bg-slate-50 p-5 rounded-3xl border border-slate-100">
                            <div className="flex items-center justify-between mb-3">
                                <label
                                  htmlFor="hasDeadline"
                                  className="text-[10px] font-black text-slate-500 uppercase tracking-widest cursor-pointer select-none flex items-center gap-2">
                                  <i className="fa-solid fa-calendar-day"></i>
                                  Date limite
                                </label>
                                <div 
                                    onClick={() => setNewMission({ ...newMission, hasDeadline: !newMission.hasDeadline })}
                                    className={`w-10 h-6 rounded-full transition-all relative cursor-pointer ${newMission.hasDeadline ? "bg-rose-500" : "bg-slate-200"}`}
                                >
                            ...
                            {newMission.hasDeadline && (
                                <input
                                  type="date"
                                  className="w-full p-3 bg-white border border-slate-200 rounded-2xl focus:border-indigo-500 outline-none transition-all font-bold text-slate-700 text-xs"
              ...
              <div className="p-8 pt-4 shrink-0 border-t border-slate-50 bg-white z-10 flex justify-end">
                <button
                  onClick={handleCreateMission}
                  className="px-12 py-4 bg-indigo-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-indigo-500/20 hover:bg-slate-900 transition-all active:scale-95 flex items-center gap-3">
