...
const UploadProcedure: React.FC<UploadProcedureProps> = ({ onBack, activeTransfer, setActiveTransfer }) => {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setActiveTransfer({ ...initialTransfer, step: "Indexation de la procédure...", progress: 80 });
      ...
      const { error: dbError } = await supabase
        ...
        .insert([
          { 
            ...
            status: 'validated',
            views: 0,
            id: filePath // On utilise le chemin comme ID pour simplifier la récupération
          }
        ]);

      if (dbError) {
        // Fallback si la table procedures n'existe pas encore ou a une structure différente, on ne bloque pas l'upload
      ...
      console.error(e);
      setErrorMsg(e.message || "Erreur lors de l'upload vers Supabase.");
      setActiveTransfer(null);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-slide-up pb-20 relative">
      ...
      {/* POPUP DE SUCCÈS */}
      {showSuccessPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-white/10 backdrop-blur-[32px] animate-fade-in">
          <div className="bg-white rounded-[4rem] p-12 max-w-lg w-full shadow-[0_40px_100px_-20px_rgba(79,70,229,0.3)] border border-white text-center space-y-10 animate-slide-up relative z-[110]">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-500 font-semibold leading-relaxed px-4">
                Le document a été sécurisé dans <span className="text-indigo-600 font-black">{selectedFolder}</span> sur Supabase.
              </p>
            </div>
            <button 
              onClick={onBack} 
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mt-2 italic">Stockage Sécurisé Supabase</p>
        </div>
      ...
      <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-xl shadow-indigo-500/5 space-y-8">
        {errorMsg && (
        ...
        <div className="flex flex-col md:flex-row items-end gap-8 pt-4">
          ...
          >
            {activeTransfer ? 'TRANSFERT EN COURS...' : 'ENREGISTRER'}
          </button>
        </div>
      </div>
    </div>
